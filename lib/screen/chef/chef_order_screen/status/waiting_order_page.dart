...
import '../../../../view/chef_waiting_item_view.dart';

class DoingOrderPage extends StatefulWidget {

  @override
  State<StatefulWidget> createState() {
    return DoingOrderPageState();
  }
}

class DoingOrderPageState extends State<DoingOrderPage> {
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<ChefViewModel>.reactive(
                ...
                children: [
                  ListView.builder(
                      ...
                      itemBuilder: (_, position) {
                        ...
                        return ChefWaitingItemView(
                          item: item, onPressed: () {
                            viewModel.send2Complete(item.id);
                        },
                          onCancelClicked: () {
      ...
      onViewModelReady: (viewModel) {
        ...
        viewModel.getOrderFood("doing");

        viewModel.errorData.listen((event) {
          showError(context, event);
        });

        viewModel.send2CompleteData.listen((event) {
          viewModel.getOrderFood("doing");
        });
